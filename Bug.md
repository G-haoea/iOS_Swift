...
* #### `The app identifier "com.iii17-grace.MyStar" cannot be registered to your development team, because it is not available. Change your bundle identifier to a unique string to try again.`     
...
* #### `Exception NSException *	"Delegate must respond to locationManager:didFailWithError:" 0x0000600002f09050`        
...
* #### `The arrowhead on storyboard disappears`    
  **`Reason`**: Forget to move arrowhead on storyboard;   
  **`Solution`**: Cheer up! Click '**Is initial viewController**' for TableViewController;   
  
* #### `Value of optional type 'UILabel?' must be unwrapped to refer to member 'text' of wrapped base type 'UILabel'`    
  **`Reason`**:  If we choose '**custom**' for TableView there will not exist a textLabel;   
  **`Solution`**: Add '**?**' instruction:     
  ```swift    
     cell.textLabel?.text = "";   
  ```    
  
* #### `Cannot be connect to repeat content`    
  **`Reason`**:  The code is not the correct one for storyboard;   
  **`Solution`**: Check the code, confirm it is the right one which we can connect objects in '**cell**' with '**cell.swift**';  
  
* #### `when click the item, the colour of background will change into grey in cell`    
  **`Reason`**:  When the item is clicked, default setting is into grey;   
  **`Solution`**: We should **deselect** it after every click by:    
  ```swift    
